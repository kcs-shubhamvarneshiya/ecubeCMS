...
jQuery(document).ready(function () {
    ...
    if (blAdd.toLowerCase() != "false") {
        shortcut.add("Ctrl+A", function () {
            window.location.href = '../Banquet/Banquet';
        });
    }
    ...
    jQuery('#btnDeleteBanquet').on('click', function () {
        DeleteItemBanquet();
    });

    jQuery('#btnExportPdf').on('click', function () {
        kcs_Common.ExportCsvPDF(false, 'Banquet', ('#txtBanquetSearch').val().trim());
    });

    jQuery('#btnExportCsv').on('click', function () {
        kcs_Common.ExportCsvPDF(true, 'Banquet', ('#txtBanquetSearch').val().trim());
    });
});

$('#txtsearch').on('click', function () {
    $('#txtBanquetSearch').toggle('slow');
    $('#txtsearch').toggleClass('close');
});
//$('#txtsearch').on('mouseover', function () {
//    $('#txtBanquetSearch').show();
//});
//$('#txtBanquetSearch').on('mouseout', function () {
//    $('#txtBanquetSearch').hide();
//});


function LoadBanquetGrid() {

    jQuery('#txtBanquetSearch').on('keyup', function (e) {
        var postData = jQuery('#tblBanquet').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtBanquetSearch').val().trim();
        jQuery('#tblBanquet').jqGrid("setGridParam", { search: true });
        jQuery('#tblBanquet').trigger("reloadGrid", [{ page: 1, current: true }]);
        SetStyle();
    });

    jQuery('#tblBanquet').jqGrid({
        url: '/Banquet/BindBanquetGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtBanquetSearch').val().trim() },
        mtype: 'GET',
        colNames: [
            'Id', 'Banquet Name', 'Description', 'Minimum Capacity', 'Maximum Capacity', 'Banquet Image', 'Terms & Conditions', 'Action'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true },
            { width: 100, name: 'BanquetName', index: 'BanquetName', align: 'left', width: '150px' },
            { name: 'Description', index: 'Description', align: 'left', width: '250px' },
            { width: 100, name: 'MinPersonCapcity', index: 'MinPersonCapcity', align: 'right', width: '150px' },
            { width: 100, name: 'MaxPersonCapcity', index: 'MaxPersonCapcity', align: 'right', width: '150px' },
            { width: 100, name: 'ProfilePic', index: 'ProfilePic', align: 'left', formatter: ViewProfileImage, width: '150px' },
            { width: 100, name: 'Terms', index: 'Terms', align: 'left', formatter: ViewTC, width: '150px' },
            { name: 'editoperation', index: 'editoperation', align: 'left', width: '100px', sortable: false, formatter: EditFormatBanquet }
            //{ name: 'deleteoperation', index: 'deleteoperation', align: 'left', width: 30, sortable: false, formatter: DeleteFormatBanquet }
        ],
        pager: jQuery('#dvBanquetFooter'),
        rowNum: kcs_Common.GridPageSize,
        rowList: kcs_Common.GridPageArray,
        sortname: 'BanquetName',
        sortorder: 'asc',
        viewrecords: true,
        caption: 'List of Banquet',
        height: '100%',
        width: '100%',
        multiselect: true,
        multiselectWidth: 50,
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() != "false") {
                window.location.href = '../Banquet/Banquet?' + rowid;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblBanquet').prev()[0].innerHTML = kcs_Message.GridNoDataFound;
            }
            else {
                $('#tblBanquet').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() == "false") {
        jQuery('#tblBanquet').jqGrid('hideCol', ['editoperation']);
    }

    if (blDelete.toLowerCase() == "false") {
        jQuery('#tblBanquet').jqGrid('hideCol', ['deleteoperation']);
    }

    SetStyle();
}

function DeleteItemBanquet(objId) {
    if (objId == undefined || objId == '') {
        var selRowIds = jQuery('#tblBanquet').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length == 0) {
            jAlert(kcs_Message.NoRecordToDelete('Banquet'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('Banquet'), function (r) {
        if (r) {
            jQuery.post("/Banquet/DeleteBanquet/", { strBanquetId: objId },
                function (data) {
                    if (data.toString() != "") {
                        jAlert(data);
                        $('#tblBanquet').trigger('reloadGrid', [{ page: 1, current: true }]);
                    }
                });
        }
    });
