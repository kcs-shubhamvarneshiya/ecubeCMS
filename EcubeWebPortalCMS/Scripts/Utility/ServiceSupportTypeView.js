...
jQuery(document).ready(function () {
    ...
    if (blAdd.toLowerCase() != "false") {
        shortcut.add("Ctrl+A", function () {
            window.location.href = '../ServiceSupportType/ServiceSupportType';
        });
    }
    ...
    LoadServiceSupportTypeGrid();

    jQuery('#btnDeleteSupport').on('click', function () {
      
        DeleteItemSupport();
    });
});

function LoadServiceSupportTypeGrid() {    
    jQuery('#txtServiceSupportTypeSearch').on('keyup', function (e) {
        var postData = jQuery('#tblServiceSupportType').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtServiceSupportTypeSearch').val().trim();
        jQuery('#tblServiceSupportType').jqGrid("setGridParam", { search: true });
        jQuery('#tblServiceSupportType').trigger("reloadGrid", [{ page: 1, current: true }]);
    });

    jQuery('#tblServiceSupportType').jqGrid({        
        url: '/ServiceSupportType/BindServiceSupportType/', 
        postData: { search: jQuery('#txtServiceSupportTypeSearch').val().trim() },
        datatype: 'json',
        mtype: 'GET',
        colNames: [
            'Id','Name', 'Description', 'Active', 'Edit', 'Delete'],
        colModel: [
        ...
        pager: jQuery('#dvServiceSupportTypeFooter'),
        ...
        multiselect: true,
        multiselectWidth: 50,
        caption: 'List of Service Support Type',
        height: '100%',
        width: '100%',
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() != "false") {
                window.location.href = '../ServiceSupportType/ServiceSupportType?' + rowid;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblServiceSupportType').prev()[0].innerHTML = kcs_Message.GridNoDataFound;
            }
            else {
                $('#tblServiceSupportType').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() == "false") {
        jQuery('#tblServiceSupportType').jqGrid('hideCol', ['editoperation']);
    }

    if (blDelete.toLowerCase() == "false") {
        jQuery('#tblServiceSupportType').jqGrid('hideCol', ['deleteoperation']);
    }

    SetStyle();
}

function DeleteItemSupport(objId) {
  
    if (objId == undefined || objId == '') {
        var selRowIds = jQuery('#tblServiceSupportType').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length == 0) {
            jAlert(kcs_Message.NoRecordToDelete('Service Support Type'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('Service Support Type'), function (r) {
        if (r) {
           
            jQuery.post("/ServiceSupportType/DeleteServiceSupportType/", { strSupportIdList: objId },
            function (data) {
                if (data.toString() != "") {
                    jAlert(data);
                    $('#tblServiceSupportType').trigger('reloadGrid');
                }
            });
        }
    });
