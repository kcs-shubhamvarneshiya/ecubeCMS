...
var names = ["SNO", "Name", "Relation", "Photo", "Adult/Child", "Amount"];
var data = [];
...
$(document).ready(function () {
    ...
    jQuery("#tblGuest").jqGrid({
        datatype: "local",
        height: 250,
        colNames: ["", "Guest Name", "Relation", "GuestImage", "Adult/Child", "IsAdult", "Amount", "Action"],
        colModel: [{
            name: 'SNO',
            index: 'SNO',
...
function DeleteItemEventSchedule(objId, rowId) {
    ...
    jConfirm(kcs_Message.DeleteConfirm('Guest Record'), function (r) {
        if (r) {   
...
function SelectedIndexChangedAdult() {
    LoadEventMemberGrid("1");
}
function LoadEventMemberGrid(Refresh) {

    var ele = document.getElementsByName('Adult');
    var radioValue = '';
    ...
    $.ajax({
        type: "GET",
        url: "/EventBookingForTicket/GetEventRate",        
        datatype: "Json",
        data: { MemberId: $('#MemberNoId').val(), EventId: parseInt($("#hdnEventID").val()), BookingDate: $('#BookingDate').val(), Type: radioValue == 1 ? 1 : 2 },        
        success: function (data) {
...
$('#btnAddGuest').click(function () {    
    ...
    if ($('#GuestName').val() == "" || $('#GuestName').val == undefined) {
        jAlert("Please Enter Guest Name.");
        return;
    }
    ...
    data[0]['SNO'] = mydata.length + 1,
        data[0]['GuestName'] = $('#GuestName').val(),
        data[0]['Relation'] = $('#hfdRelationId').val(),
        data[0]['GuestImage'] = $("#imgCapture")[0].src;
    data[0]['IsAdult'] = radioValue,
        data[0]['Adult/Child'] = radioValue == 1 ? 'Adult' : 'Child',
...
$('#btnSave').click(function () {  
    ...
    if ($("#MemberNoId").val() == undefined || $("#MemberNoId").val() == 0) {
    ...
    if (mydata.length == 0) {
        jAlert("Please add atleast one guest.");
        return;
    }
    ...
    for (var i = 0; i < mydata.length; i++) {       
     ...
     if (total != parseFloat($("#txtTotal").val())) {
        jAlert("Tatal amount and added guest amount total does not match.");
        return;
    }
    ...
    var requestData = {};
    requestData.MemberId = $('#MemberNoId').val();
    //requestData.GuestName= $('#GuestName').val();
    //requestData.IsAdult= radioValue;
    ...
    requestData.GuestList = mydata;
        ...
        $.ajax({
            url: "/EventBookingForTicket/SaveEventBookingForGuest",
            type: 'POST',
