...
jQuery(document).ready(function () {
    ...
    if (blAdd.toLowerCase() != "false") {
        shortcut.add("Ctrl+A", function () {
            window.location.href = '../RoomBooking/RoomBooking';
        });
    }
    ...
    jQuery('#btnDeleteRoomBooking').on('click', function () {
        DeleteItemRoomBooking();
    });

    jQuery('#btnExportPdf').on('click', function () {
        kcs_Common.ExportCsvPDF(false, 'RoomBooking', ('#txtRoomBookingSearch').val().trim());
    });

    jQuery('#btnExportCsv').on('click', function () {
        kcs_Common.ExportCsvPDF(true, 'RoomBooking', ('#txtRoomBookingSearch').val().trim());
    });
});

$('#txtsearch').on('click', function () {
    $('#txtRoomBookingSearch').toggle('slow');
    $('#txtsearch').toggleClass('close');
});
//$('#txtsearch').on('mouseover', function () {
//    $('#txtRoomBookingSearch').show();
//});
//$('#txtRoomBookingSearch').on('mouseout', function () {
//    $('#txtRoomBookingSearch').hide();
//});

function LoadRoomBookingGrid() {

    jQuery('#txtRoomBookingSearch').on('keyup', function (e) {
        var postData = jQuery('#tblRoomBooking').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtRoomBookingSearch').val().trim();
        jQuery('#tblRoomBooking').jqGrid("setGridParam", { search: true });
        jQuery('#tblRoomBooking').trigger("reloadGrid", [{ page: 1, current: true }]);
        SetStyle();
    });

    jQuery('#tblRoomBooking').jqGrid({
        url: '/RoomBooking/BindRoomBookingGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtRoomBookingSearch').val().trim() },
        mtype: 'GET',
        colNames: [
            'Id', 'Room Type', 'Description', 'Member Rate', 'Guest Rate', 'Room Image', 'Terms & Conditions', 'Action'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true },
            { name: 'RoomBookingName', index: 'RoomBookingName', align: 'left', width: '150px' },
            { width: 250, name: 'Description', index: 'Description', align: 'left', width: '250px' },
            { width: 75, name: 'Member', index: 'Member', align: 'right', width: '150px' },
            { width: 75, name: 'Guest', index: 'Guest', align: 'right', width: '150px' },
            { name: 'ProfilePic', index: 'ProfilePic', align: 'left', formatter: ViewProfileImage, width: '150px' },
            { width: 100, name: 'Terms', index: 'Terms', align: 'left', formatter: ViewTC, width: '200px' },
            { name: 'editoperation', index: 'editoperation', align: 'left', width: '100px', sortable: false, formatter: EditFormatRoomBooking }
            //{ name: 'deleteoperation', index: 'deleteoperation', align: 'left', width: 40, sortable: false, formatter: DeleteFormatRoomBooking }
        ],
        pager: jQuery('#dvRoomBookingFooter'),
        rowNum: kcs_Common.GridPageSize,
        rowList: kcs_Common.GridPageArray,
        sortname: 'RoomBookingName',
        sortorder: 'asc',
        viewrecords: true,
        caption: 'List of Room Booking',
        height: '100%',
        width: '100%',
        multiselect: true,
        multiselectWidth: 50,
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() != "false") {
                window.location.href = '../RoomBooking/RoomBooking?' + rowid;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblRoomBooking').prev()[0].innerHTML = kcs_Message.GridNoDataFound;
            }
            else {
                $('#tblRoomBooking').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() == "false") {
        jQuery('#tblRoomBooking').jqGrid('hideCol', ['editoperation']);
    }

    if (blDelete.toLowerCase() == "false") {
        jQuery('#tblRoomBooking').jqGrid('hideCol', ['deleteoperation']);
    }

    SetStyle();
}

function DeleteItemRoomBooking(objId) {
    if (objId == undefined || objId == '') {
        var selRowIds = jQuery('#tblRoomBooking').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length == 0) {
            jAlert(kcs_Message.NoRecordToDelete('Room Booking'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('Room Booking'), function (r) {
        if (r) {
            jQuery.post("/RoomBooking/DeleteRoomBooking/", { strRoomBookingId: objId },
                function (data) {
                    if (data.toString() != "") {
                        jAlert(data);
                        $('#tblRoomBooking').trigger('reloadGrid', [{ page: 1, current: true }]);
                    }
                });
            }
        });
