...
jQuery(document).ready(function () {
    $('#txtFacilityCategorySearch').hide();
    ...
    LoadFacilityCategoryGrid();

    $(window).bind('resize', function () {
        SetStyle();
    });

    jQuery('#btnDeleteFacilityCategory').on('click', function () {
        DeleteItemFacilityCategory();
    });

    jQuery('#btnExportPdf').on('click', function () {
        kcs_Common.ExportCsvPDF(false, 'FacilityCategory', ('#txtFacilityCategorySearch').val().trim());
    });

    jQuery('#btnExportCsv').on('click', function () {
        kcs_Common.ExportCsvPDF(true, 'FacilityCategory', ('#txtFacilityCategorySearch').val().trim());
    });
});

$('#txtsearch').on('click', function () {
    $('#txtFacilityCategorySearch').toggle('slow');
    $('#txtsearch').toggleClass('close');
});


//$('#txtsearch').on('mouseover', function () {
//    $('#txtFacilityCategorySearch').show();
//});
//$('#txtFacilityCategorySearch').on('mouseout', function () {
//    $('#txtFacilityCategorySearch').hide();
//});

function LoadFacilityCategoryGrid() {

    jQuery('#txtFacilityCategorySearch').on('keyup', function (e) {
        var postData = jQuery('#tblFacilityCategory').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtFacilityCategorySearch').val().trim();
        jQuery('#tblFacilityCategory').jqGrid("setGridParam", { search: true });
        jQuery('#tblFacilityCategory').trigger("reloadGrid", [{ page: 1, current: true }]);
        //SetStyle();
    });

    jQuery('#tblFacilityCategory').jqGrid({
        url: '/FacilityCategory/BindFacilityCategoryGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtFacilityCategorySearch').val().trim() },
        mtype: 'GET',
        colNames: [
            'Id', 'Facility Category Name', 'Sequence No', 'Action'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true },
            { name: 'Name', index: 'Name', align: 'left', width: '600px' },
            { name: 'SequenceNo', index: 'SequenceNo', align: 'left', width: '200px' },
            { name: 'editoperation', index: 'editoperation', align: 'left', width: '100px', sortable: false, formatter: EditFormatFacilityCategory }
            //{ name: 'deleteoperation', index: 'deleteoperation', align: 'left', width: 40, sortable: false, formatter: DeleteFormatFacilityCategory }
        ],
        pager: jQuery('#dvFacilityCategoryFooter'),
        rowNum: kcs_Common.GridPageSize,
        rowList: kcs_Common.GridPageArray,
        sortname: 'Id',
        sortorder: 'DESC',
        viewrecords: true,
        caption: 'List of FacilityCategory',
        height: '100%',
        width: '100%',
        multiselect: true,
        multiselectWidth: 50,
        ...
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() !== "false") {
                window.location.href = '../FacilityCategory/FacilityCategory?' + rowid;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblFacilityCategory').prev()[0].innerHTML = kcs_Message.GridNoDataFound;
            }
            else {
                $('#tblFacilityCategory').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() === "false") {
        jQuery('#tblFacilityCategory').jqGrid('hideCol', ['editoperation']);
    }

    if (blDelete.toLowerCase() === "false") {
        jQuery('#tblFacilityCategory').jqGrid('hideCol', ['deleteoperation']);
    }

    SetStyle();
}

function DeleteItemFacilityCategory(objId) {
    if (objId == undefined || objId === '') {
        var selRowIds = jQuery('#tblFacilityCategory').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length === 0) {
            jAlert(kcs_Message.NoRecordToDelete('FacilityCategory'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('FacilityCategory'), function (r) {
        if (r) {
            jQuery.post("/FacilityCategory/DeleteFacilityCategory/", { strCategoryId: objId },
                function (data) {
                    if (data.toString() !== "") {
                        jAlert(data);
                        $('#tblFacilityCategory').trigger('reloadGrid', [{ page: 1, current: true }]);
                    }
                });
        }
    });
