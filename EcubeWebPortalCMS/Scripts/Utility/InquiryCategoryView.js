...
jQuery(document).ready(function () {
    ...
    if (blAdd.toLowerCase() != "false") {
        shortcut.add("Ctrl+A", function () {
            window.location.href = '../InquiryCategory/InquiryCategory';
        });
    }
    ...
    LoadInquiryCategoryGrid();

    jQuery('#btnDeleteInquiryCategory').on('click', function () {
        DeleteFormatInquiryCategory();
    });

    jQuery('#btnExportPdf').on('click', function () {
        kcs_Common.ExportCsvPDF(false, 'InquiryCategory', ('#txtInquiryCategorySearch').val().trim());
    });

    jQuery('#btnExportCsv').on('click', function () {
        kcs_Common.ExportCsvPDF(true, 'InquiryCategory', ('#txtInquiryCategorySearch').val().trim());
    });
});

function LoadInquiryCategoryGrid() {
    jQuery('#txtInquiryCategorySearch').on('keyup', function (e) {
        var postData = jQuery('#tblInquiryCategory').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtInquiryCategorySearch').val().trim();
        jQuery('#tblInquiryCategory').jqGrid("setGridParam", { search: true });
        jQuery('#tblInquiryCategory').trigger("reloadGrid", [{ page: 1, current: true }]);
        SetStyle();
    });
    jQuery('#tblInquiryCategory').jqGrid({
        url: '/InquiryCategory/BindInquiryCategoryGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtInquiryCategorySearch').val().trim() },
        mtype: 'GET',
        colNames: [
            'Id', 'Inquiry Category', 'Mobile No.', 'Email', 'Sequence No', 'Action'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true },
            { name: 'Category', index: 'Category', align: 'left', width: '200px' },
            { name: 'MobileNo', index: 'MobileNo', align: 'left', width: '200px' },
            { name: 'Email', index: 'Email', align: 'left', width: '200px' },
            { name: 'SeqNo', index: 'SeqNo', align: 'left', width: '200px' },
            { name: 'editoperation', index: 'editoperation', align: 'left', width: '100px', sortable: false, formatter: EditFormatInquiryCategory },
            //{ name: 'deleteoperation', index: 'deleteoperation', align: 'left', width: 40, sortable: false, formatter: DeleteFormatInquiryCategory }
        ],
        pager: jQuery('#dvInquiryCategoryFooter'),
        rowNum: kcs_Common.GridPageSize,
        rowList: kcs_Common.GridPageArray,
        sortname: 'Id',
        sortorder: 'desc',
        viewrecords: true,
        caption: 'List of Inquiry Category',
        height: '100%',
        width: '100%',
        multiselect: true,
        multiselectWidth: 50,
        ...
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() != "false") {
                window.location.href = '../InquiryCategory/InquiryCategory?' + rowid;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblInquiryCategory').prev()[0].innerHTML = kcs_Message.GridNoDataFound;
            }
            else {
                $('#tblInquiryCategory').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() == "false") {
        jQuery('#tblInquiryCategory').jqGrid('hideCol', ['editoperation']);
    }

    if (blDelete.toLowerCase() == "false") {
        jQuery('#tblInquiryCategory').jqGrid('hideCol', ['deleteoperation']);
    }
...
$('#txtsearch').on('click', function () {
    $('#txtInquiryCategorySearch').val('');
    $('#txtInquiryCategorySearch').toggle('slow');
    $('#txtsearch').toggleClass('close');
    jQuery('#txtInquiryCategorySearch').trigger('keyup');
});

function DeleteFormatInquiryCategory(objId) {
    if (objId == undefined || objId == '') {
        var selRowIds = jQuery('#tblInquiryCategory').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length == 0) {
            jAlert(kcs_Message.NoRecordToDelete('Inquiry Category'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('Inquiry Category'), function (r) {
        if (r) {
            jQuery.post("/InquiryCategory/DeleteInquiryCategory/", { strInquiryCategoryId: objId },
                function (data) {
                    if (data.toString() != "") {
                        jAlert(data);
                        $('#tblInquiryCategory').trigger('reloadGrid', [{ page: 1, current: true }]);
                    }
                });
        }
    });
