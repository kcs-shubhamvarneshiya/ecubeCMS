...
jQuery(document).ready(function () {
    ...
    jQuery('#btnDeleteFacilityGroup').on('click', function () {
        DeleteItemFacilityGroup();
    });

    jQuery('#btnExportPdf').on('click', function () {
        kcs_Common.ExportCsvPDF(false, 'FacilityGroup', ('#txtFacilityCategorySearch').val().trim());
    });

    jQuery('#btnExportCsv').on('click', function () {
        kcs_Common.ExportCsvPDF(true, 'FacilityGroup', ('#txtFacilityCategorySearch').val().trim());
    });
});

$('#txtsearch').on('click', function () {
    $('#txtFacilityGroupSearch').toggle('slow');
    $('#txtsearch').toggleClass('close');
});

//$('#txtsearch').on('mouseover', function () {
//    $('#txtFacilityGroupSearch').show();
//});
//$('#txtFacilityGroupSearch').on('mouseout', function () {
//    $('#txtFacilityGroupSearch').hide();
//});

function LoadFacilityCategoryGrid() {

    jQuery('#txtFacilityGroupSearch').on('keyup', function (e) {
        var postData = jQuery('#tblFacilityGroup').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtFacilityGroupSearch').val().trim();
        jQuery('#tblFacilityGroup').jqGrid("setGridParam", { search: true });
        jQuery('#tblFacilityGroup').trigger("reloadGrid", [{ page: 1, current: true }]);
        //SetStyle();
    });

    jQuery('#tblFacilityGroup').jqGrid({
        url: '/FacilityCategory/BindFacilityGroupGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtFacilityGroupSearch').val().trim() },
        mtype: 'GET',
        colNames: [
            'Id', 'Facility', 'Facility Category Name', 'Sequence No', 'Action'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true },
            { name: 'Name', index: 'Name', align: 'left', width: '300px' },
            { name: 'FacilityName', index: 'FacilityName', align: 'left', width: '400px' },
            { name: 'SequenceNo', index: 'SequenceNo', align: 'left', width: '150px',  },
        ...
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() !== "false") {
                window.location.href = '../FacilityCategory/FacilityGroup?' + rowid;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblFacilityGroup').prev()[0].innerHTML = kcs_Message.GridNoDataFound;
            }
            else {
                $('#tblFacilityGroup').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() === "false") {
        jQuery('#tblFacilityGroup').jqGrid('hideCol', ['editoperation']);
    }

    if (blDelete.toLowerCase() === "false") {
        jQuery('#tblFacilityGroup').jqGrid('hideCol', ['deleteoperation']);
    }

    SetStyle();
}

function DeleteItemFacilityGroup(objId) {
    if (objId == undefined || objId === '') {
        var selRowIds = jQuery('#tblFacilityGroup').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length === 0) {
            jAlert(kcs_Message.NoRecordToDelete('FacilityGroup'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('FacilityGroup'), function (r) {
        if (r) {
            jQuery.post("/FacilityCategory/DeleteFacility/", { strFacilityId: objId },
                function (data) {
                    if (data.toString() !== "") {
                        jAlert(data);
                        $('#tblFacilityGroup').trigger('reloadGrid', [{ page: 1, current: true }]);
                    }
                });
        }
    });
...
function SetStyle() {
    $('#tblFacilityGroup').setGridWidth($('#dvFacilityGroupFooter').width());
}