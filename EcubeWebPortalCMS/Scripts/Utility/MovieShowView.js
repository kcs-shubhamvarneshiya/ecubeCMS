...
function LoadMovieShowGrid() {
    ...
    jQuery('#tblMovieShow').jqGrid({
        url: '/MovieShow/BindMovieShowGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtMovieShowSearch').val().trim() },
        mtype: 'GET',
        colNames: [
            'Id', 'Movie Show Name', 'Show Start Time', 'Show End Time', 'Action'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true, },
            { name: 'MovieShowName', index: 'MovieShowName', align: 'left', width: '400px' },
            { name: 'ShowStartDate', index: 'ShowStartDate', align: 'left', width: '250px' },
            { name: 'ShowEndDate', index: 'ShowEndDate', align: 'left', width: '250px'  },
        ...
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() != "false") {
                window.location.href = '../MovieShow/MovieShow?' + rowid;
            }
        },
...
function DeleteItemMovieShow(objId) {
    ...
    jConfirm(kcs_Message.DeleteConfirm('Movie Show'), function (r) {
        if (r) {
            jQuery.post("/MovieShow/DeleteMovieShow/", { strMovieShowId: objId },
                function (data) {
                    if (data.toString() != "") {
                        jAlert(data);
                        /*$('#tblMovieShow').trigger('reloadGrid', [{ page: 1, current: true }]);*/
                        $("#tblMovieShow").clearGridData(true).trigger(
