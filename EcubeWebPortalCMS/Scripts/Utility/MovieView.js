...
jQuery(document).ready(function () {
    ...
    jQuery('#btnDeleteMovie').on('click', function () {
        DeleteItemMovie();
    });

    jQuery('#btnExportPdf').on('click', function () {
        kcs_Common.ExportCsvPDF(false, 'Movie', ('#txtMovieSearch').val().trim());
    });

    jQuery('#btnExportCsv').on('click', function () {
        kcs_Common.ExportCsvPDF(true, 'Movie', ('#txtMovieSearch').val().trim());
    });
});
function LoadMovieGrid() {

    jQuery('#txtMovieSearch').on('keyup', function (e) {
        var postData = jQuery('#tblMovie').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtMovieSearch').val().trim();
        postData.isActive = $("#ActiveMovie:checkbox:checked").length > 0 ? true : false;
        jQuery('#tblMovie').jqGrid("setGridParam", { search: true });
        jQuery('#tblMovie').trigger("reloadGrid", [{ page: 1, current: true }]);
        SetStyle();
    });

    $('#ActiveMovie').change(function () {
        var postData = jQuery('#tblMovie').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtMovieSearch').val().trim();
        postData.isActive = $("#ActiveMovie:checkbox:checked").length > 0 ? 'true' : 'false';
        jQuery('#tblMovie').jqGrid("setGridParam", { search: true });
    ...
    jQuery('#tblMovie').jqGrid({
        url: '/Movie/BindMovieGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtMovieSearch').val().trim(), isActive: $("#ActiveMovie:checkbox:checked").length > 0 ? true : false },
        mtype: 'GET',
        colNames: [
            'Id', 'Movie Name', 'Movie Category', 'Movie Rating', 'Movie Language', 'Movie Content Type', 'Movie Type', 'Action'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true, width: '100px' },
            { name: 'MovieName', index: 'MovieName', align: 'left', width: '80px'},
            { name: 'MovieCategory', index: 'MovieCategory', align: 'left', width: '100px'},
            { name: 'MovieRating', index: 'MovieRating', align: 'left', width: '100px'},
            { name: 'MovieLanguage', index: 'MovieLanguage', align: 'left', width: '120px'},
            { name: 'MovieContentType', index: 'MovieContentType', align: 'left', width: '110px'},
            { name: 'MovieType', index: 'MovieType', align: 'left', width: '80px'},
            { name: 'editoperation', index: 'editoperation', align: 'left', width: '100px',sortable: false, formatter: EditFormatMovie }
            //{ name: 'deleteoperation', index: 'deleteoperation', align: 'left', width: 70, sortable: false, formatter: DeleteFormatMovie }
        ],
        pager: jQuery('#dvMovieFooter'),
        ...
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() != "false") {
                window.location.href = '../Movie/Movie?' + rowid;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblMovie').prev()[0].innerHTML = kcs_Message.GridNoDataFound;
            }
            else {
                $('#tblMovie').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() == "false") {
        jQuery('#tblMovie').jqGrid('hideCol', ['editoperation']);
    }

    if (blDelete.toLowerCase() == "false") {
        jQuery('#tblMovie').jqGrid('hideCol', ['deleteoperation']);
    }
    SetStyle();
}

function DeleteItemMovie(objId) {   
    if (objId == undefined || objId == '') {
        var selRowIds = jQuery('#tblMovie').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length == 0) {
            jAlert(kcs_Message.NoRecordToDelete('Movie'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('Movie'), function (r) {
        if (r) {
            jQuery.post("/Movie/DeleteMovie/", { strMovieId: objId },
                function (data) {
                    if (data.toString() != "") {
                        jAlert(data);
                        //$('#tblMovie').trigger('reloadGrid', [{ page: 1, current: true }]);
                        $("#tblMovie").clearGridData(true).trigger(
