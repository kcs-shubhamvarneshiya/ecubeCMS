...
jQuery(document).ready(function () {
    ...
    if (blAdd.toLowerCase() != "false") {
        shortcut.add("Ctrl+A", function () {
            window.location.href = '../EventCategory/EventCategory';
        });
    }
    ...
    jQuery('#btnDeleteEventCategory').on('click', function () {
        DeleteItemEventCategory();
    });

    jQuery('#btnExportPdf').on('click', function () {
        kcs_Common.ExportCsvPDF(false, 'EventCategory', ('#txtEventCategorySearch').val().trim());
    });

    jQuery('#btnExportCsv').on('click', function () {
        kcs_Common.ExportCsvPDF(true, 'EventCategory', ('#txtEventCategorySearch').val().trim());
    });
});
function LoadEventCategoryGrid() {

    jQuery('#txtEventCategorySearch').on('keyup', function (e) {
        var postData = jQuery('#tblEventCategory').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtEventCategorySearch').val().trim();
        jQuery('#tblEventCategory').jqGrid("setGridParam", { search: true });
        jQuery('#tblEventCategory').trigger("reloadGrid", [{ page: 1, current: true }]);
        //SetStyle();
    });

    jQuery('#tblEventCategory').jqGrid({
        url: '/EventCategory/BindEventCategoryGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtEventCategorySearch').val().trim() },
        mtype: 'GET',
        colNames: [
            'Id', 'Event Category Name', 'Action'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true },
            { name: 'EventCategoryName', index: 'EventCategoryName', align: 'left', width: '900px' },
            { name: 'editoperation', index: 'editoperation', align: 'left', width: '100px', sortable: false, formatter: EditFormatEventCategory }
            //{ name: 'deleteoperation', index: 'deleteoperation', align: 'left', width: 40, sortable: false, formatter: DeleteFormatEventCategory }
        ],
        pager: jQuery('#dvEventCategoryFooter'),
        rowNum: kcs_Common.GridPageSize,
        rowList: kcs_Common.GridPageArray,
        sortname: 'EventCategoryName',
        sortorder: 'asc',
        viewrecords: true,
        caption: 'List of Event Category',
        height: '100%',
        width: '100%',
        multiselect: true,
        multiselectWidth: 50,
        ...
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() != "false") {
                window.location.href = '../EventCategory/EventCategory?' + rowid;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblEventCategory').prev()[0].innerHTML = kcs_Message.GridNoDataFound;
            }
            else {
                $('#tblEventCategory').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() == "false") {
        jQuery('#tblEventCategory').jqGrid('hideCol', ['editoperation']);
    }

    if (blDelete.toLowerCase() == "false") {
        jQuery('#tblEventCategory').jqGrid('hideCol', ['deleteoperation']);
    }
...
$('#txtsearch').on('click', function () {
    $('#txtEventCategorySearch').toggle('slow');
    $('#txtsearch').toggleClass('close');
});

//$('#txtsearch').on('mouseover', function () {
//    $('#txtEventCategorySearch').show();
//});
//$('#txtEventCategorySearch').on('mouseout', function () {
//    $('#txtEventCategorySearch').hide();
//});

function DeleteItemEventCategory(objId) {
    if (objId == undefined || objId == '') {
        var selRowIds = jQuery('#tblEventCategory').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length == 0) {
            jAlert(kcs_Message.NoRecordToDelete('Event Category'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('Event Category'), function (r) {
        if (r) {
            jQuery.post("/EventCategory/DeleteEventCategory/", { strEventCategoryId: objId },
                function (data) {
                    if (data.toString() != "") {
                        jAlert(data);
                        $('#tblEventCategory').trigger('reloadGrid', [{ page: 1, current: true }]);
                    }
                });
            }
        });
