...
jQuery(document).ready(function () {
    ...
    if (blAdd.toLowerCase() != "false") {
        shortcut.add("Ctrl+A", function () {
            window.location.href = '../CMSConfig/CMSConfig';
        });
    }
    ...
    jQuery('#btnDeleteCMSConfig').on('click', function () {
        DeleteItemCMSConfig();
    });

    jQuery('#btnExportPdf').on('click', function () {
        kcs_Common.ExportCsvPDF(false, 'CMSConfig', ('#txtCMSConfigSearch').val().trim());
    });

    jQuery('#btnExportCsv').on('click', function () {
        kcs_Common.ExportCsvPDF(true, 'CMSConfig', ('#txtCMSConfigSearch').val().trim());
    });
});
$('#txtsearch').on('click', function () {
    $('#txtCMSConfigSearch').toggle('slow');
    $('#txtsearch').toggleClass('close');
});
//$('#txtsearch').on('mouseover', function () {
//    $('#txtCMSConfigSearch').show();
//});
//$('#txtCMSConfigSearch').on('mouseout', function () {
//    $('#txtCMSConfigSearch').hide();
//});

function LoadCMSConfigGrid() {

    jQuery('#txtCMSConfigSearch').on('keyup', function (e) {
        var postData = jQuery('#tblCMSConfig').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtCMSConfigSearch').val().trim();
        jQuery('#tblCMSConfig').jqGrid("setGridParam", { search: true });
        jQuery('#tblCMSConfig').trigger("reloadGrid", [{ page: 1, current: true }]);
        SetStyle();
    });

    jQuery('#tblCMSConfig').jqGrid({
        url: '/CMSConfig/BindCMSConfigGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtCMSConfigSearch').val().trim() },
        mtype: 'GET',
        colNames: [
            'Id', 'Module', 'Person Name', 'Email Id', 'Mobile No', 'Action'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true },
            { name: 'Module', index: 'Module', align: 'left', width: '150px' },
            { name: 'PersonName', index: 'PersonName', align: 'left', width: '250px' },
            { name: 'EmailID', index: 'EmailID', align: 'left', width: '250px' },
            { name: 'MobileNo', index: 'MobileNo', align: 'left', width: '150px' },
            { name: 'editoperation', index: 'editoperation', align: 'left', width: '100px', sortable: false, formatter: EditFormatCMSConfig }
            //{ name: 'deleteoperation', index: 'deleteoperation', align: 'left', width: 40, sortable: false, formatter: DeleteFormatCMSConfig }
        ],
        pager: jQuery('#dvCMSConfigFooter'),
        rowNum: kcs_Common.GridPageSize,
        rowList: kcs_Common.GridPageArray,
        sortname: 'CMSConfigName',
        sortorder: 'asc',
        viewrecords: true,
        ...
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() != "false") {
                window.location.href = '../CMSConfig/CMSConfig?' + rowid;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblCMSConfig').prev()[0].innerHTML = kcs_Message.GridNoDataFound;
            }
            else {
                $('#tblCMSConfig').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() == "false") {
        jQuery('#tblCMSConfig').jqGrid('hideCol', ['editoperation']);
    }

    if (blDelete.toLowerCase() == "false") {
        jQuery('#tblCMSConfig').jqGrid('hideCol', ['deleteoperation']);
    }

    SetStyle();
}

function DeleteItemCMSConfig(objId) {
    if (objId == undefined || objId == '') {
        var selRowIds = jQuery('#tblCMSConfig').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length == 0) {
            jAlert(kcs_Message.NoRecordToDelete('Configuration'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('Configuration'), function (r) {
        if (r) {
            jQuery.post("/CMSConfig/DeleteCMSConfig/", { strCMSConfigId: objId },
                function (data) {
                    if (data.toString() != "") {
                        jAlert(data);
                        $('#tblCMSConfig').trigger('reloadGrid', [{ page: 1, current: true }]);
                    }
                });
            }
        });
