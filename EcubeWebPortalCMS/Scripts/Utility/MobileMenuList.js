...
jQuery(document).ready(function () {
    ...
    jQuery('#btnDeleteMenu').on('click', function () {
        DeleteMobileMenu();
    });

    jQuery('#btnExportPdf').on('click', function () {
        kcs_Common.ExportCsvPDF(false, 'MobileMenu', ('#txtMenuListSearch').val().trim());
    });

    jQuery('#btnExportCsv').on('click', function () {
        kcs_Common.ExportCsvPDF(true, 'MobileMenu', ('#txtMenuListSearch').val().trim());
    });
});


$(document).ready(function () {
    jQuery('#txt_dvMenuListFooter').prop('disabled', true);
    $("#txtsearch").click(function () {        
        $('#txtMenuListSearch').val('');
        $('#txtMenuListSearch').toggle('slow');
        $('#txtsearch').toggleClass('close'); 
        jQuery('#txtMenuListSearch').trigger('keyup');
    });
});

function LoadFacilityCategoryGrid() {    
    jQuery('#txtMenuListSearch').on('keyup', function (e) {
        var postData = jQuery('#tblMenuList').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtMenuListSearch').val().trim();
        jQuery('#tblMenuList').jqGrid("setGridParam", { search: true });
        jQuery('#tblMenuList').trigger("reloadGrid", [{ page: 1, current: true }]);
        //SetStyle();
    });

    jQuery('#tblMenuList').jqGrid({
        url: '/MenuMaster/BindMenuMasterGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtMenuListSearch').val().trim() },
        mtype: 'GET',
        colNames: [
            'MenuId', 'Sr No', 'Menu Key', 'Menu Display Name', 'Sequence No', 'Sub Menu', 'Action'],
        colModel: [
            { name: 'MenuId', index: 'MenuId', align: 'left', key: true, hidden: true },            
            { name: 'RowNum', index: 'RowNum', align: 'left', width: '100px' },            
            { name: 'MenuKey', index: 'SequenceNo', align: 'left', width: '200px' },
            { name: 'MenuDisplayName', index: 'SequenceNo', align: 'left', width: '200px' },
            { name: 'SeqNo', index: 'SequenceNo', align: 'left', width: '200px' },
            { name: 'SubMenu', index: 'SubMenu', align: 'left', width: '300px' },
            { name: 'editoperation', index: 'editoperation', align: 'left', width: '100px', sortable: false, formatter: EditFormatMenu }
            //{ name: 'deleteoperation', index: 'deleteoperation', align: 'left', width: 40, sortable: false, formatter: DeleteFormatFacilityCategory }
        ],
        pager: jQuery('#dvMenuListFooter'),
        ...
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() !== "false") {
                window.location.href = '../MenuMaster/MenuMobile?' + rowid;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblMenuList').prev()[0].innerHTML = kcs_Message.GridNoDataFoundv2;
            }
            else {
                $('#tblMenuList').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() === "false") {
        jQuery('#tblMenuList').jqGrid('hideCol', ['editoperation']);
    }

    if (blDelete.toLowerCase() === "false") {
        jQuery('#tblMenuList').jqGrid('hideCol', ['deleteoperation']);
    }

    SetStyle();
}

function DeleteMobileMenu(objId) {    
    if (objId == undefined || objId === '') {
        var selRowIds = jQuery('#tblMenuList').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length === 0) {
            jAlert(kcs_Message.NoRecordToDelete('MobileMenu'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('MobileMenu'), function (r) {       
        if (r) {
            jQuery.post("/MenuMaster/DeleteMobileMenu/", { strMenuId: objId },
                function (data) {
                    if (data.toString() !== "") {
                        jAlert(data);
                        $('#tblMenuList').trigger('reloadGrid', [{ page: 1, current: true }]);
                    }
                });
        }
    });
