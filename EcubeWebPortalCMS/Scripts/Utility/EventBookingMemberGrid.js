...
jQuery(document).ready(function () {
    ...
    if (blAdd.toLowerCase() != "false") {
        shortcut.add("Ctrl+A", function () {
            window.location.href = '../EventBookingForTicket/EventBookingForTicket';
        });
    }
...
function LoadEventBookingGrid() {
    ...
    jQuery('#tblEventBooking').jqGrid({
        ...
        mtype: 'GET',
        colNames: [
            'Id', 'Member Name', 'Relation', 'Age', 'Photo', 'MemberFees', 'Step', 'Action'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true },
            { name: 'MemberName', index: 'MemberName', align: 'left', width: '200px' },
            { name: 'Relation', index: 'Relation', align: 'left', width: '200px' },
            { name: 'Age', index: 'Age', align: 'left', width: '200px' },
            { name: 'Photo', index: 'Photo', align: 'left', width: '100px' },
            { name: 'MemberFees', index: 'MemberFees', align: 'left', width: '200px' },
            //{ name: 'MemberFees', index: 'MemberFees', width: '200px', align: 'left', formatter: 'date', formatoptions: { srcformat: 'ts', newformat: 'd/m/Y' } },
        ...
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() != "false") {
                if ($("#chkEventComplete").is(':checked') == false) {
                    ...
                    window.location.href = '../EventBookingForTicket/EventBookingForTicket?' + rowid;
...
function DeleteItemEventBooking(objId) {
    ...
    jConfirm(kcs_Message.DeleteConfirm('Event'), function (r) {
        if (r) {
            jQuery.post("/EventBookingForTicket/DeleteEvent/", { strEventId: objId },
                function (data) {
                    if (data.toString() != "") {
                        jAlert(data);
