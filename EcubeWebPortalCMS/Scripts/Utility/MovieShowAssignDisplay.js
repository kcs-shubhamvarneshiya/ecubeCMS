...
jQuery(document).ready(function () {
    ...
    if (blAdd.toLowerCase() != "false") {
        shortcut.add("Ctrl+A", function () {
            window.location.href = '../MovieShow/MovieShow';
        });
    }
    ...
    jQuery('#btnDeleteMovieShow').on('click', function () {
        DeleteItemMovieShow();
    });

    jQuery('#btnExportPdf').on('click', function () {
        kcs_Common.ExportCsvPDF(false, 'MovieShow', ('#txtMovieShowSearch').val().trim());
    });

    jQuery('#btnExportCsv').on('click', function () {
        kcs_Common.ExportCsvPDF(true, 'MovieShow', ('#txtMovieShowSearch').val().trim());
    });
});
function LoadMovieShowGrid() {
    jQuery('#txtMovieShowSearch').on('keyup', function (e) {
        var postData = jQuery('#tblMovieShow').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtMovieShowSearch').val().trim();
        postData.displayPastRecord = $("#OldRecord:checkbox:checked").length > 0 ? 'true' : 'false';
        jQuery('#tblMovieShow').jqGrid("setGridParam", { search: true });
        jQuery('#tblMovieShow').trigger("reloadGrid", [{ page: 1, current: true }]);
        SetStyle();
    });

    $('#OldRecord').change(function () {
        var postData = jQuery('#tblMovieShow').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtMovieShowSearch').val().trim();
        postData.displayPastRecord = $("#OldRecord:checkbox:checked").length > 0 ? 'true' : 'false';
        jQuery('#tblMovieShow').jqGrid("setGridParam", { search: true });
    ...
    var displayPastRecord = $("#OldRecord:checkbox:checked").length > 0 ? 'true' : 'false';
    jQuery('#tblMovieShow').jqGrid({
        url: '/MovieShowAssign/BindMovieShowGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtMovieShowSearch').val().trim(), displayPastRecord: displayPastRecord },
        mtype: 'GET',
        colNames: [
            'Id', 'Movie Theatre Name', 'Show Start Time', 'Show End Time', 'Action'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true},
            { name: 'MovieTheatreName', index: 'MovieTheatreName', align: 'left', width: '300px' },
            { name: 'ShowStartDate', index: 'ShowStartDate', align: 'left', width: '300px' },
            { name: 'ShowEndDate', index: 'ShowEndDate', align: 'left', width: '300px' },
            { name: 'editoperation', index: 'editoperation', align: 'left', width: '100px', sortable: false, formatter: EditFormatMovieShow }
            //{ name: 'deleteoperation', index: 'deleteoperation', align: 'left', width: 60, sortable: false, formatter: DeleteFormatMovieShow }
        ],
        pager: jQuery('#dvMovieShowFooter'),
        rowNum: kcs_Common.GridPageSize,
        rowList: kcs_Common.GridPageArray,
        sortname: 'ShowStartDate',
        sortorder: 'desc',
        viewrecords: true,
        caption: 'List of Movie Show',
        height: '100%',
        width: '100%',
        multiselect: false,
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() != "false") {
                var isOld = $("#OldRecord:checkbox:checked").length > 0 ? 'true' : 'false';
                window.location.href = '../MovieShowAssign/MovieShowAssignView?SID=' + rowid + '&IsOld=' + isOld ;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblMovieShow').prev()[0].innerHTML = kcs_Message.GridNoDataFound;
            }
            else {
                $('#tblMovieShow').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() == "false") {
        jQuery('#tblMovieShow').jqGrid('hideCol', ['editoperation']);
    }

    if (blDelete.toLowerCase() == "false") {
        jQuery('#tblMovieShow').jqGrid('hideCol', ['deleteoperation']);
    }

    SetStyle();
}
$('#txtsearch').on('click', function () {
    $('#txtMovieShowSearch').toggle('slow');
    $('#txtsearch').toggleClass('close');
});
//$('#txtMovieShowSearch').on('click', function () {
//    $('#txtMovieShowSearch').hide('slow');
//});

function DeleteMovieAssignShow(objId) {
    if (objId == undefined || objId == '') {
        var selRowIds = jQuery('#tblMovieShow').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length == 0) {
            jAlert(kcs_Message.NoRecordToDelete('Movie Show'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('Movie Show'), function (r) {
        if (r) {
            jQuery.post("/MovieShowAssign/DeleteMovieAssignShow/", { showPeriodId: objId },
                function (data) {
                    if (data.toString() != "") {
                        jAlert(data);
                        $('#tblMovieShow').trigger('reloadGrid', [{ page: 1, current: true }]);
                    }
                });
        }
    });
