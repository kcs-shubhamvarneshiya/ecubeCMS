...
jQuery(document).ready(function () {
    $('#txtEventTicketCategorySearch').hide();
    ...
    if (blAdd.toLowerCase() != "false") {
        shortcut.add("Ctrl+A", function () {
            window.location.href = '../EventTicketCategory/EventTicketCategory';
        });
    }
    ...
    LoadEventTicketCategoryGrid();

    jQuery('#btnDeleteEventTicketCategory').on('click', function () {
        DeleteItemEventTicketCategory();
    });

    jQuery('#btnExportPdf').on('click', function () {
        kcs_Common.ExportCsvPDF(false, 'EventTicketCategory', ('#txtEventTicketCategorySearch').val().trim());
    });

    jQuery('#btnExportCsv').on('click', function () {
        kcs_Common.ExportCsvPDF(true, 'EventTicketCategory', ('#txtEventTicketCategorySearch').val().trim());
    });
});

$('#txtsearch').on('click', function () {
    $('#txtEventTicketCategorySearch').toggle('slow');
    $('#txtsearch').toggleClass('close');
});

//$('#txtsearch').on('mouseover', function () {
//    $('#txtEventTicketCategorySearch').show();
//});
//$('#txtEventTicketCategorySearch').on('mouseout', function () {
//    $('#txtEventTicketCategorySearch').hide();
//});


function LoadEventTicketCategoryGrid() {

    jQuery('#txtEventTicketCategorySearch').on('keyup', function (e) {
        var postData = jQuery('#tblEventTicketCategory').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtEventTicketCategorySearch').val().trim();
        jQuery('#tblEventTicketCategory').jqGrid("setGridParam", { search: true });
        jQuery('#tblEventTicketCategory').trigger("reloadGrid", [{ page: 1, current: true }]);
        //SetStyle();
    });

    jQuery('#tblEventTicketCategory').jqGrid({
        url: '/EventTicketCategory/BindEventTicketCategoryGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtEventTicketCategorySearch').val().trim() },
        mtype: 'GET',
        colNames: [
            'Id', 'Event Ticket Category Name','Action'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true },
            { name: 'EventTicketCategoryName', index: 'EventTicketCategoryName', align: 'left', width: '900px', },
            { name: 'editoperation', index: 'editoperation', align: 'left', width: '100px', sortable: false, formatter: EditFormatEventTicketCategory }
            //{ name: 'deleteoperation', index: 'deleteoperation', align: 'left', width: 40, sortable: false, formatter: DeleteFormatEventTicketCategory }
        ],
        pager: jQuery('#dvEventTicketCategoryFooter'),
        rowNum: kcs_Common.GridPageSize,
        rowList: kcs_Common.GridPageArray,
        sortname: 'EventTicketCategoryName',
        sortorder: 'asc',
        viewrecords: true,
        caption: 'List of Event Ticket Category',
        height: '100%',
        width: '100%',
        multiselect: true,
        multiselectWidth: 50,
        ...
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() != "false") {
                window.location.href = '../EventTicketCategory/EventTicketCategory?' + rowid;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblEventTicketCategory').prev()[0].innerHTML = kcs_Message.GridNoDataFound;
            }
            else {
                $('#tblEventTicketCategory').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() == "false") {
        jQuery('#tblEventTicketCategory').jqGrid('hideCol', ['editoperation']);
    }

    if (blDelete.toLowerCase() == "false") {
        jQuery('#tblEventTicketCategory').jqGrid('hideCol', ['deleteoperation']);
    }

    SetStyle();
}

function DeleteItemEventTicketCategory(objId) {
    if (objId == undefined || objId == '') {
        var selRowIds = jQuery('#tblEventTicketCategory').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length == 0) {
            jAlert(kcs_Message.NoRecordToDelete('Event Ticket Category'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('Event Ticket Category'), function (r) {
        if (r) {
            jQuery.post("/EventTicketCategory/DeleteEventTicketCategory/", { strEventTicketCategoryId: objId },
                function (data) {
                    if (data.toString() != "") {
                        jAlert(data);
                        $('#tblEventTicketCategory').trigger('reloadGrid', [{ page: 1, current: true }]);
                    }
                });
            }
        });
