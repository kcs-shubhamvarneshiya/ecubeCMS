...
jQuery(document).ready(function () {
    ...
    jQuery('#btnDeleteInformation').on('click', function () {
        DeleteItemInformation();
    });

    jQuery('#btnExportPdf').on('click', function () {
        kcs_Common.ExportCsvPDF(false, 'Information', ('#txtInformationSearch').val().trim());
    });

    jQuery('#btnExportCsv').on('click', function () {
        kcs_Common.ExportCsvPDF(true, 'Information', ('#txtInformationSearch').val().trim());
    });
});

$('#txtsearch').on('click', function () {
    $('#txtInformationSearch').toggle('slow');
    $('#txtsearch').toggleClass('close');
});


//$('#txtsearch').on('mouseover', function () {
//    $('#txtInformationSearch').show();
//});
//$('#txtInformationSearch').on('mouseout', function () {
//    $('#txtInformationSearch').hide();
//});


function LoadInformationGrid() {

    jQuery('#txtInformationSearch').on('keyup', function (e) {
        var postData = jQuery('#tblInformation').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtInformationSearch').val().trim();
        jQuery('#tblInformation').jqGrid("setGridParam", { search: true });
        jQuery('#tblInformation').trigger("reloadGrid", [{ page: 1, current: true }]);
        SetStyle();
    });

    jQuery('#tblInformation').jqGrid({
        url: '/Information/BindInformationGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtInformationSearch').val().trim() },
        mtype: 'GET',
        colNames: [
            'Id', 'Information Name', 'Sequence No', 'Action'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true },
            { name: 'InformationName', index: 'InformationName', align: 'left', width: '600px', },
            { name: 'SequenceNo', index: 'SequenceNo', align: 'left', width: '200px', },
            { name: 'editoperation', index: 'editoperation', align: 'left', width: '100px', sortable: false, formatter: EditFormatInformation },
            //{ name: 'deleteoperation', index: 'deleteoperation', align: 'left', width: 60, sortable: false, formatter: DeleteFormatInformation }
        ],
        pager: jQuery('#dvInformationFooter'),
        ...
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() !== "false") {
                window.location.href = '../Information/Information?' + rowid;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblInformation').prev()[0].innerHTML = kcs_Message.GridNoDataFound;
            }
            else {
                $('#tblInformation').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() === "false") {
        jQuery('#tblInformation').jqGrid('hideCol', ['editoperation']);
    }

    if (blDelete.toLowerCase() === "false") {
        jQuery('#tblInformation').jqGrid('hideCol', ['deleteoperation']);
    }

    SetStyle();
}

function DeleteItemInformation(objId) {
    if (objId == undefined || objId === '') {
        var selRowIds = jQuery('#tblInformation').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length === 0) {
            jAlert(kcs_Message.NoRecordToDelete('Information'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('Information'), function (r) {
        if (r) {
            jQuery.post("/Information/DeleteInformation/", { strInformationId: objId },
                function (data) {
                    if (data.toString() !== "") {
                        jAlert(data);
                        $('#tblInformation').trigger('reloadGrid', [{ page: 1, current: true }]);
                    }
                });
        }
    });
