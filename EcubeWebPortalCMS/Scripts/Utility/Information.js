$(document).ready(function () {
    ...
    $('.cancel').click(function () {
        if ($('#HdnIFrame').val().toLowerCase() == 'false') {
            window.location.href = 'InformationView';
        }
        else {
    ...
    DisplayMessage('InformationView');
    ...
    $('#btnSubmit').click(function () {
        if ($('#Name').val().trim() === '') {
            jAlert(kcs_Message.InputRequired('Information Name'), 'Name');
            return false;
        }
            ...
            if (typeof (fileUpload.files) !== "undefined") {
                ...
                reader.onload = function (e) {
                    ...
                    image.onload = function () {
                        var postdata = $('#frmInformation').serialize();
                        var file = document.getElementById('ImagePath').files[0];
                        var fd = new FormData();
                        fd.append("files", file);
                        var xhr = new XMLHttpRequest();
                        xhr.open("POST", "/Information/UploadImage", false);
                        xhr.send(fd);
