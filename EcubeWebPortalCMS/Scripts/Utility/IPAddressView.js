...
jQuery(document).ready(function () {
    ...
    if (blAdd.toLowerCase() != "false") {
        shortcut.add("Ctrl+A", function () {
            window.location.href = '../IPAddress/IPAddress';
        });
    }
    ...
    jQuery('#btnDeleteIPAddress').on('click', function () {
        DeleteItemIPAddress();
    });
});

function LoadIPAddressGrid() {

    jQuery('#txtIPAddressSearch').on('keyup', function (e) {
        var postData = jQuery('#tblIPAddress').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtIPAddressSearch').val().trim();
        jQuery('#tblIPAddress').jqGrid("setGridParam", { search: true });
        jQuery('#tblIPAddress').trigger("reloadGrid", [{ page: 1, current: true }]);
        SetStyle();
    });

    jQuery('#tblIPAddress').jqGrid({
        url: '/IPAddress/BindIPAddressGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtIPAddressSearch').val().trim() },
        mtype: 'GET',
        colNames: [
            'Id', 'IP Address', 'Description', 'Is Active', 'Edit', 'Delete'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true },
            { name: 'IPAddressName', index: 'IPAddressName', align: 'left' },
            { name: 'Description', index: 'Description', align: 'left' },
            { name: 'IsActive', index: 'IsActive', align: 'center', formatter: 'checkbox', sortable: false },
            { name: 'editoperation', index: 'editoperation', align: 'left', width: 40, sortable: false, formatter: EditFormatIPAddress },
            { name: 'deleteoperation', index: 'deleteoperation', align: 'left', width: 40, sortable: false, formatter: DeleteFormatIPAddress }
        ],
        pager: jQuery('#dvIPAddressFooter'),
        rowNum: kcs_Common.GridPageSize,
        rowList: kcs_Common.GridPageArray,
        sortname: 'IPAddressName',
        sortorder: 'asc',
        viewrecords: true,
        caption: 'List of IP  Address',
        height: '100%',
        width: '100%',
        multiselect: true,
        multiselectWidth: 50,
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() != "false") {
                window.location.href = '../IPAddress/IPAddress?' + rowid;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblIPAddress').prev()[0].innerHTML = kcs_Message.GridNoDataFound;
            }
            else {
                $('#tblIPAddress').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() == "false") {
        jQuery('#tblIPAddress').jqGrid('hideCol', ['editoperation']);
    }
    if (blDelete.toLowerCase() == "false") {
        jQuery('#tblIPAddress').jqGrid('hideCol', ['deleteoperation']);
    }

    SetStyle();
}

function DeleteItemIPAddress(objId) {
    if (objId == undefined || objId == '') {
        var selRowIds = jQuery('#tblIPAddress').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length == 0) {
            jAlert(kcs_Message.NoRecordToDelete('IP Address'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('IP Address'), function (r) {
        if (r) {
            jQuery.post("/IPAddress/DeleteIPAddress/", { strIPAddressId: objId },
                function (data) {
                    if (data.toString() != "") {
                        jAlert(data);
                        $('#tblIPAddress').trigger('reloadGrid', [{ page: 1, current: true }]);
                    }
                });
        }
    });
