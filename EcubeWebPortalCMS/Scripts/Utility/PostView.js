...
jQuery(document).ready(function () {
    ...
    jQuery('#btnDeletePost').on('click', function () {
        DeleteItemPost();
    });

    jQuery('#btnExportPdf').on('click', function () {
        kcs_Common.ExportCsvPDF(false, 'Post', ('#txtPostSearch').val().trim());
    });

    jQuery('#btnExportCsv').on('click', function () {
        kcs_Common.ExportCsvPDF(true, 'Post', ('#txtPostSearch').val().trim());
    });
});

$('#txtsearch').on('click', function () {
    $('#txtPostSearch').toggle('slow');
...
//    $('#txtPostSearch').show();
//});
//$('#txtPostSearch').on('mouseout', function () {
//    $('#txtPostSearch').hide();
//});
...
function LoadPostGrid() {
    debugger
    jQuery('#txtPostSearch').on('keyup', function (e) {
        var postData = jQuery('#tblPost').jqGrid("getGridParam", "postData");
        postData.search = jQuery('#txtPostSearch').val().trim();
        jQuery('#tblPost').jqGrid("setGridParam", { search: true });
        jQuery('#tblPost').trigger("reloadGrid", [{ page: 1, current: true }]);
        SetStyle();
    });

    jQuery('#tblPost').jqGrid({
        url: '/Post/BindPostGrid/',
        datatype: 'json',
        postData: { search: jQuery('#txtPostSearch').val().trim() },
        mtype: 'GET',
        colNames: [
            'Id', 'Post Name', 'Description', 'Action'],
        colModel: [
            { name: 'Id', index: 'Id', align: 'left', key: true, hidden: true },
            { name: 'PostName', index: 'PostName', align: 'left', width: '350px' },
            { name: 'PostDescription', index: 'PostDescription', align: 'left', width: '500px' },
            { name: 'editoperation', index: 'editoperation', align: 'left', width: '100px', sortable: false, formatter: EditFormatPost }
            //{ name: 'deleteoperation', index: 'deleteoperation', align: 'left', width: 40, sortable: false, formatter: DeleteFormatPost }
        ],
        pager: jQuery('#dvPostFooter'),
        ...
        ondblClickRow: function (rowid) {
            if (blEdit.toLowerCase() !== "false") {
                window.location.href = '../Post/Post?' + rowid;
            }
        },
        loadComplete: function (data) {
            if (data.records == 0) {
                $('#tblPost').prev()[0].innerHTML = kcs_Message.GridNoDataFound;
            }
            else {
                $('#tblPost').prev()[0].innerHTML = '';
            }
    ...
    if (blEdit.toLowerCase() === "false") {
        jQuery('#tblPost').jqGrid('hideCol', ['editoperation']);
    }

    if (blDelete.toLowerCase() === "false") {
        jQuery('#tblPost').jqGrid('hideCol', ['deleteoperation']);
    }

    SetStyle();
}

function DeleteItemPost(objId) {
    if (objId == undefined || objId === '') {
        var selRowIds = jQuery('#tblPost').jqGrid('getGridParam', 'selarrrow');
        if (selRowIds.length === 0) {
            jAlert(kcs_Message.NoRecordToDelete('Post'));
            return false;
        }
    ...
    jConfirm(kcs_Message.DeleteConfirm('Post'), function (r) {
        if (r) {
            jQuery.post("/Post/DeletePost/", { strPostId: objId },
                function (data) {
                    if (data.toString() !== "") {
                        jAlert(data);
                        $('#tblPost').trigger('reloadGrid', [{ page: 1, current: true }]);
                    }
                });
        }
    });
